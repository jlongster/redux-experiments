...
/***/ function(module, exports, __webpack_require__) {
	...
	var App = React.createClass({
	  ...
	  render: function render() {
	    return dom.div(null, Link({ to: "/item?foo=bar" }, "This is an app."), this.props.children);
	  }
	});

	React.render(Router({ history: history,
	  onUpdate: function onUpdate() {
	    console.log(this.history.path);
	  } }, Route({ path: "/", component: App }, Route({ path: "item", component: Item }))), document.querySelector('#mount'));
	  ...
	  _updateState: function _updateState(location) {
	    ...
	    runTransition(this.state, this.routes, location, hooks, function (error, state, transition) {
	      if (error) {
	        ...
	        _this.setState(state, function(arg) {
	          _this.props.onUpdate(arg)
	        });
	      }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var History = (function () {
	  ...
	  History.prototype.onBeforeChange = function onBeforeChange(listener) {
	    _warning2['default'](this.beforeChangeListener != null, 'beforeChange listener of History should not be overwritten');

	    this.beforeChangeListener = listener;
	  };

	  History.prototype.setup = function setup(path) {
	    var entry = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];

	    if (this.location) return;

	    if (!entry.key) entry = this.replace(path, this.createRandomKey());

	    var state = null;
	    if (typeof this.readState === 'function') state = this.readState(entry.key);

	    var location = this._createLocation(path, state, entry, _NavigationTypes2['default'].POP);
	  ...
	  History.prototype.handlePop = function handlePop(path) {
	    var _this = this;

	    var entry = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];

	    var state = null;
	    if (entry.key && typeof this.readState === 'function') state = this.readState(entry.key);

	    var pendingLocation = this._createLocation(path, state, entry, _NavigationTypes2['default'].POP);

	    this.beforeChange(pendingLocation, function () {
	      _this._update(path, pendingLocation);
	    });
	  };

	  History.prototype.createRandomKey = function createRandomKey() {
	    return Math.random().toString(36).substr(2);
	  };

	  History.prototype._saveNewState = function _saveNewState(state) {
	    var key = this.createRandomKey();

	    if (state != null) {
	      _invariant2['default'](typeof this.saveState === 'function', '%s needs a saveState method in order to store state', this.constructor.name);
	  ...
	  History.prototype.canUpdateState = function canUpdateState() {
	    return typeof this.readState === 'function' && typeof this.saveState === 'function' && this.location && this.location.state && this.location.state.key;
	  };

	  History.prototype.updateState = function updateState(extraState) {
	    _invariant2['default'](this.canUpdateState(), '%s is unable to update state right now', this.constructor.name);

	    var key = this.location.state.key;
	    var state = this.readState(key);
	    this.saveState(key, _extends({}, state, extraState));
	  };

	  History.prototype.beforeChange = function beforeChange(location, done) {
	    var _this2 = this;
	    ...
	    if (!this.beforeChangeListener) {
	      ...
	      this.beforeChangeListener.call(this, location, function () {
	        if (_this2._pendingLocation === location) {
	  ...
	  History.prototype.isPending = function isPending(location) {
	    return this._pendingLocation === location;
	  };

	  History.prototype.pushState = function pushState(state, path) {
	    var _this3 = this;

	    var pendingLocation = this._createLocation(path, state, null, _NavigationTypes2['default'].PUSH);
	    this.beforeChange(pendingLocation, function () {
	      _this3._doPushState(state, path);
	    });
	  };

	  History.prototype._doPushState = function _doPushState(state, path) {
	    ...
	    _warning2['default'](entry.key || state == null, '%s does not support storing state', this.constructor.name);

	    var location = this._createLocation(path, state, entry, _NavigationTypes2['default'].PUSH);
	    this._update(path, location);
	  };

	  History.prototype.replaceState = function replaceState(state, path) {
	    var _this4 = this;

	    var pendingLocation = this._createLocation(path, state, null, _NavigationTypes2['default'].REPLACE);
	    this.beforeChange(pendingLocation, function () {
	      _this4._doReplaceState(state, path);
	    });
	  };

	  History.prototype._doReplaceState = function _doReplaceState(state, path) {
	    var key = this._saveNewState(state);
	    var entry = this.replace(path, key) || {};

	    _warning2['default'](entry.key || state == null, '%s does not support storing state', this.constructor.name);

	    var location = this._createLocation(path, state, entry, _NavigationTypes2['default'].REPLACE);
	    this._update(path, location);
	  };
	  ...
	  History.prototype._update = function _update(path, location) {
	    var notify = arguments.length <= 2 || arguments[2] === undefined ? true : arguments[2];
	  ...
	  History.prototype._createLocation = function _createLocation(path, state, entry, navigationType) {
	    ...
	    return new _Location2['default'](pathname, query, _extends({}, state, entry), navigationType);
	  };
...
/***/ function(module, exports, __webpack_require__) {
	...
	var BrowserHistory = (function (_DOMHistory) {
	  ...
	  BrowserHistory.prototype.setup = function setup() {
	    ...
	    var key = null;
	    if (this.isSupported && window.history.state) key = window.history.state.key;

	    _DOMHistory.prototype.setup.call(this, path, { key: key });

	    _DOMUtils.addEventListener(window, 'popstate', this.handlePopState);
	  };

	  BrowserHistory.prototype.teardown = function teardown() {
	    _DOMUtils.removeEventListener(window, 'popstate', this.handlePopState);
	    _DOMHistory.prototype.teardown.call(this);
	  };
	  ...
	  BrowserHistory.prototype.handlePopState = function handlePopState(event) {
	    ...
	    this.handlePop(path, { key: key });
	  };

	  // http://www.w3.org/TR/2011/WD-html5-20110113/history.html#dom-history-pushstate

	  BrowserHistory.prototype.push = function push(path, key) {
	    if (this.isSupported) {
	      var state = { key: key };
	  ...
	  BrowserHistory.prototype.replace = function replace(path, key) {
	    if (this.isSupported) {
	      var state = { key: key };
...
/***/ function(module, exports, __webpack_require__) {
	...
	var DOMHistory = (function (_History) {
	  ...
	  DOMHistory.prototype.setup = function setup(path, entry) {
	    _History.prototype.setup.call(this, path, entry);
	    _DOMUtils.addEventListener(window, 'beforeunload', this.handleBeforeUnload);
	  };

	  DOMHistory.prototype.teardown = function teardown() {
	    _DOMUtils.removeEventListener(window, 'beforeunload', this.handleBeforeUnload);
	    _History.prototype.teardown.call(this);
	  };
	  ...
	  DOMHistory.prototype.saveState = function saveState(key, state) {
	    window.sessionStorage.setItem(key, JSON.stringify(state));
	  };

	  DOMHistory.prototype.readState = function readState(key) {
	  ...
	  DOMHistory.prototype.beforeChange = function beforeChange(location, done) {
	    var _this = this;

	    _History.prototype.beforeChange.call(this, location, function () {
	      if (location.navigationType === _NavigationTypes2['default'].PUSH && _this.canUpdateState()) {
	        var scrollPosition = _this.getScrollPosition();
	        _this.updateState(scrollPosition);
	      }
	      done();
	    });
	  };

	  DOMHistory.prototype.handleBeforeUnload = function handleBeforeUnload(event) {
